...
public class BuildManager: MonoBehaviour 
{
   ...
   // public Snapping snapping = new Snapping();
            ...
            if (hit[i].collider.tag == TerrainCollisionTag)
			{
				...
				if (ghost != null)
				{
                    if (Input.GetMouseButtonDown(0) && !ghostCollision.Collided() && isFlat)
                    {
                        ...
                        new Vector3(
                                Mathf.RoundToInt(hit[i].point.x*2)/2f,
                                0, //hit[i].point.y + Building[SelectedBuilding].GetComponent<Collider>().transform.localScale.y / 2,
                                Mathf.RoundToInt(hit[i].point.z*2)/2f),
                        Quaternion.identity);

                        string s = bl.buildingGameObject.name.Replace("(Clone)", "");
                        bl.buildingGameObject.name = s;
                        bl.buildingName = s;
                        bl.buildingGameObject.transform.parent = towerParent;
					...
					{
						ghost.GetComponentInChildren<Renderer>().material.CopyPropertiesFromMaterial(Building[SelectedBuilding].GetComponentInChildren<Renderer>().sharedMaterial);
						ghost.GetComponentInChildren<Renderer>().material.color = new Color(
							1f,
							0f, 
							0f, 
							0.6f);
    ...
    public void StartPlacingStone()
	{
        SelectBuilding(0);
        ActivateBuildingMode();
	}

    public void AddBuilding(BuildingList bl)
	{
        Stone stone = bl.buildingGameObject.GetComponent<Stone>();
        if (stone != null) stone.TransformStone();
        ...
        if (buildingCount >= controller.buildingsPerLevel)
		{
            buildingCount = 0;
